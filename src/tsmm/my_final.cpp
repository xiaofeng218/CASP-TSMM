...
// Forward declarations: optimized tiling kernels from my_blocking.cpp
...
// ============================================================
// my_final: Shape-adaptive dispatcher
//
//   Tiny m×n (≤256) + large k → thread-private + k-parallel + SIMD
//   Everything else           → optimized tiling (delegated)
// ============================================================
...
void tsmm_my_final(int m, int n, int k,
    ...
    if (layout == Layout::RowMajor) {
        if (tiny && big_k)
            final_tiny_row(m, n, k, A, B, C);
        else
            blocking_tiled_row(m, n, k, A, B, C);
    } else {
        if (tiny && big_k)
            final_tiny_col(m, n, k, A, B, C);
        else
